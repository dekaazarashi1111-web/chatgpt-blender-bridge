...
def smooth_union(a, b, radius):
    # Log-sum-exp smooth minimum has no finite-support second-derivative seam.
    return np.minimum(a,b)-radius*np.log1p(np.exp(-np.abs(a-b)/radius))
...
def head_field(x,y,z):
    cranium=ellipsoid(x,y,z,(0,-.036,1.797),(.206,.201,.204))
    bridge=ellipsoid(x,y,z,(0,-.217,1.743),(.051,.052,.116))
    field=smooth_union(cranium,bridge,.011)
    for sign in (-1,1):
        # Cheek volume is part of the continuous face, rather than a glued oval.
        cheek=ellipsoid(x,y,z,(sign*.179,-.106,1.718),(.065,.150,.097))
        field=smooth_union(field,cheek,.010)
        rise=ellipsoid(x,y,z,(sign*.060,-.233,1.710),(.078,.070,.103))
        field=smooth_union(field,rise,.019)
        cushion=ellipsoid(x,y,z,(sign*.054,-.267,1.655),(.079,.065,.052))
        field=smooth_union(field,cushion,.019)
    cleft=ellipsoid(x,y,z,(0,-.335,1.616),(.0032,.036,.040))
    field=np.maximum(field,-cleft)
    mouth=ellipsoid(x,y,z,(0,-.236,1.531),(.151,.171,.072))
    field=np.maximum(field,-mouth)
    for sign in (-1,1):
        orbit=ellipsoid(x,y,z,(sign*.069,-.231,1.823),(.044,.081,.051))
        field=np.maximum(field,-orbit)
    return field


def front_surface(x,z):
    # First front-facing crossing of the exact field, vectorized over points.
    x=np.asarray(x,dtype=float);z=np.asarray(z,dtype=float)
    ys=np.linspace(-.400,.030,180)
    q=head_field(x[:,None],ys[None,:],z[:,None])
    inside=q<0;has=inside.any(axis=1)
    if not has.all():raise ValueError('Cheek cover leaves the face domain')
    index=inside.argmax(axis=1);lo=ys[np.maximum(index-1,0)];hi=ys[index]
    for _ in range(18):
        mid=(lo+hi)*.5;f=head_field(x,mid,z)
