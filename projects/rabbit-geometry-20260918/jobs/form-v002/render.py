"""Exact-scale, head-isolated geometry reviews, with explicit camera-frustum checks."""
...
image('head_front',(0,-7,1.535),(0,-.10,1.535),.60,1200,head_names)
image('head_right',(7,-.10,1.535),(0,-.10,1.535),.62,1200,head_names)
image('head_three_quarter',(3,-6,2.6),(0,-.08,1.53),.68,1200,head_names)
image('head_uniform_clay',(3,-6,2.6),(0,-.08,1.53),.68,1200,head_names,True)
foot_names={o.name for o in bpy.data.collections['07_FEET'].objects}
image('feet_detail',(2,-4,2.2),(0,-.07,.072),.87,1000,foot_names)
hand_names={o.name for o in bpy.data.collections['05_HANDS'].objects if o.name.endswith('L')}
image('hand_detail',(2.2,-2.8,2.25),(.885,0,1.23),.45,1000,hand_names)
...
checkpoint('revised-review-views','Frustum-checked orthographic views and isolated head, hand and foot details rendered by Blender')
