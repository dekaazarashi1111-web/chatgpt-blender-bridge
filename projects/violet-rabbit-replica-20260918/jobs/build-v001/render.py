...
scene.render.engine='CYCLES'; scene.cycles.device='CPU'; scene.cycles.samples=int(PARAMS.get('samples',72))
...
env.inputs['Color'].default_value=(.42,.42,.42,1);env.inputs['Strength'].default_value=.5
...
for name,loc,power,size in [('Studio_key',(-3,-4,5),500,4),('Studio_fill',(4,-2,3),150,5),('Studio_back',(0,3,4),250,4)]:
...
data=bpy.data.lights.new('Eye_pin_light','POINT');data.energy=4;data.shadow_soft_size=.018
...
size=int(PARAMS.get('resolution',768));scene.render.resolution_x=size;scene.render.resolution_y=size;scene.render.resolution_percentage=100
...
specs=[('front',(0,-6,1.303),(0,0,1.303),2.678),('right',(6,0,1.303),(0,0,1.303),2.678),
       ...
       ('face',(0,-6,1.823),(0,-.05,1.823),.655),('face_profile',(6,-.02,1.823),(0,-.06,1.823),.72),
...
(Path(OUTPUT_DIR)/'render_settings.json').write_text(json.dumps({'renderer':bpy.app.version_string,'views':records,'lighting':'neutral soft studio','review':'not yet performed'},indent=2))
