"""Revision 2: lossless PNG-derived specimens and material-masked source projection."""
import hashlib
import json
import math
import shutil
...
def quilt(exemplar, size=320, patch=28, overlap=10):
...
specs = {
    'purple': ((1620, 328, 1668, 376), 1.42),
    'pale': ((378, 365, 426, 413), 1.46),
    'inner': ((370, 69, 383, 125), 1.35),
}
lossless = {}
sample_hashes = {'purple': '40153af37914113d5c67d6cfb55236ef05ae19a4cbf945500d35b1110fd2f85e',
                 'pale': 'f13651d417ab46e31c5ad9f3220ae1d3904995fc7ad6375f73cadf72075bef79',
                 'inner': '80d0d6896c6da30b4e5c8ce4115612b572b684e9758f922b17ec71c0bc0b4879'}
for key, digest in sample_hashes.items():
    path = Path(INPUT_DIR) / (key + '_lossless.webp')
    assert hashlib.sha256(path.read_bytes()).hexdigest() == digest
    lossless[key] = Image.open(path).convert('RGB')
...
report = {'source_sha256': hashlib.sha256(reference.read_bytes()).hexdigest(),
          ...
          'lossless_input_sha256': sample_hashes, 'revision_note': '48x48 original-PNG pigment specimens, 13x56 ear colour. Broader palette still uses the original verified WebP reference.', 'materials': {}}
for key, (box, gain) in specs.items():
    crop = lossless[key].copy()
    crop.save(OUT / (key + '_exemplar.png'))
    if min(crop.size) < 40:
        crop = crop.resize((max(48, crop.width), max(48, crop.height)), Image.Resampling.LANCZOS)
    texture = periodic(quilt(lossless['purple'] if key == 'inner' else crop))
    if key == 'inner':
        target = np.asarray(lossless['inner'], np.float32).mean((0, 1)) / 255
        texture = np.clip(texture / texture.mean((0, 1)) * target, 0, 1)
    else:
        palette_box = (1603,310,1698,401) if key == 'purple' else (365,352,439,426)
        target = np.asarray(ref.crop(palette_box), np.float32).mean((0,1)) / 255
        texture = np.clip(texture / texture.mean((0,1)) * target, 0, 1)
    ...
    report['materials'][key] = {'source_crop_xyxy': list(box), 'source_pixels': list(lossless[key].size),
...
for key, box in {'front': (64, 0, 736, 682), 'back': (1312, 0, 1984, 682), 'side': (896, 0, 1196, 682)}.items():
    ...
    # Suppress reference-background, button, bow and cross-material ghosts.
    rgb = np.asarray(ref.crop(box), np.float32)
    lum = rgb.mean(axis=2)
    for kind in ['purple', 'pale']:
        if kind == 'purple':
            valid = (rgb[:,:,0]-rgb[:,:,1] > 6) & (rgb[:,:,2]-rgb[:,:,1] > 5) & (lum > 38)
        else:
            valid = (np.abs(rgb[:,:,0]-rgb[:,:,1]) < 18) & (np.abs(rgb[:,:,2]-rgb[:,:,1]) < 18) & (lum > 100)
        mask = Image.fromarray(np.uint8(valid)*255).filter(ImageFilter.GaussianBlur(.65))
        colour = np.uint8(np.clip(lin_to_srgb(srgb_to_lin(a)*1.44),0,1)*255+.5)
        rgba = np.concatenate([colour,np.asarray(mask)[:,:,None]],axis=2)
        Image.fromarray(rgba,'RGBA').save(OUT / ('projection_'+kind+'_'+key+'.png'))
...
checkpoint('surfaces', 'Lossless specimen maps and material-masked reference projections saved')
