"""Blue animatronic: editable, rigid-rigged reference interpretation.
Front is -Y; Z is up. Job entrypoint owns saving, previews and validation.
No external assets, filesystem access, network calls or subprocesses.
"""
import bpy
import math
...
scene['revision'] = 'blue-animatronic-v001'
scene['model_notes'] = 'Separate hard-surface shells, rigid armature, no external textures. Visual review required.'
parts = []
...
def material(name, rgb, roughness=0.38, metallic=0.0, coat=0.15):
    m = bpy.data.materials.new(name)
    m.diffuse_color = tuple(linear(c) for c in rgb) + (1.0,)
    m.use_nodes = True
...
BLUE = material('01 | Azure blue enamel', (0.075, 0.445, 0.855), 0.34, 0.05, 0.22)
PALE = material('02 | Ice blue panels', (0.715, 0.855, 0.985), 0.4)
BLACK = material('03 | Charcoal soft parts', (0.095, 0.108, 0.13), 0.62, 0.0, 0.0)
SEAM = material('04 | Deep blue panel seams', (0.025, 0.09, 0.175), 0.48)
METAL = material('05 | Headphone hardware', (0.215, 0.23, 0.255), 0.36, 0.35)
WHITE = material('06 | Porcelain teeth and hat ribbon', (0.955, 0.975, 1.0), 0.28)
GLASS = material('07 | Opaque sunglass lenses', (0.009, 0.017, 0.032), 0.17, 0.05, 0.4)
SHINE = material('08 | Graphic white lens reflection', (0.975, 0.995, 1.0), 0.2)
shine_bs = SHINE.node_tree.nodes.get('Principled BSDF')
if 'Emission Color' in shine_bs.inputs:
    shine_bs.inputs['Emission Color'].default_value = (0.8, 0.87, 1.0, 1.0)
    shine_bs.inputs['Emission Strength'].default_value = 0.18
...
def soften(obj, width=0.025, segments=3):
...
def ellipsoid(name, center, scale, mat, bone='head', exponent=1.0):
    bpy.ops.mesh.primitive_uv_sphere_add(segments=40, ring_count=24, radius=1.0, location=center)
...
def cylinder(name, center, radius, depth, mat, bone='head', axis=(0, 0, 1), rtop=None):
    bpy.ops.mesh.primitive_cone_add(vertices=64, radius1=radius,
        radius2=radius if rtop is None else rtop, depth=depth, location=center)
    obj = bpy.context.object
    obj.rotation_euler = Vector(axis).to_track_quat('Z', 'Y').to_euler()
    register(obj, name, mat, bone)
    return soften(obj, min(0.018, depth * 0.15), 3)
...
def ring(name, center, rx, ry, radius, mat, bone='torso', axis=(0,0,1)):
    ...
    c = Vector(center)
    pts = [tuple(c + q @ Vector((rx*math.cos(t), ry*math.sin(t), 0)))
        for t in [2*math.pi*i/64 for i in range(64)]]
    return pipe(name, pts, radius, mat, bone, closed=True)
...
def catmull(rows, subdivisions=5):
    ...
    for i in range(len(rows)-1):
        ...
        for step in range(subdivisions):
            t = step/subdivisions
            output.append(tuple(0.5*((2*b)+(-a+c)*t+(2*a-5*b+4*c-d)*t*t+(-a+3*b-3*c+d)*t*t*t)
                for a,b,c,d in zip(p0,p1,p2,p3)))
    output.append(tuple(rows[-1]))
    return output


def loft(name, rows, mat, bone='torso', exponent=1.0, sides=64):
    ...
    for z, rx, ry, cy in profiles:
        for j in range(sides):
            ...
            verts.append((max(rx,0.006)*math.copysign(abs(co)**exponent,co),
                cy+max(ry,0.006)*math.copysign(abs(si)**exponent,si), z))
...
def prism(name, outline, front_y, thickness, mat, bone='head', bevel=0.02):
    n = len(outline)
...
def segment(name, start, end, radius, mat, bone):
    ...
    length = (b-a).length
    rows = [(0,radius*.64,radius*.64,0),(.06*length,radius*.87,radius*.87,0),
        (.24*length,radius,radius,0),(.66*length,radius*.98,radius*.98,0),
        (.92*length,radius*.78,radius*.78,0),(length,radius*.65,radius*.65,0)]
...
# Main shell dimensions taken from the front / side reference proportions.
body_rows = [(2.64,.84,.49,.04),(2.72,1.015,.61,.04),(2.95,1.04,.64,.045),
    (3.35,1.005,.66,.05),(3.82,.94,.64,.055),(4.18,.83,.565,.05),
    (4.40,.68,.45,.04),(4.47,.50,.33,.03)]
body = loft('Torso | blue outer shell', body_rows, BLUE, 'torso', exponent=.86)
head_rows = [(4.39,.22,.18,-.43),(4.46,.39,.31,-.35),(4.68,.55,.44,-.20),
    (4.92,.84,.64,.04),(5.15,1.025,.745,.10),(5.38,.965,.75,.13),
    (5.66,.845,.72,.13),(5.95,.785,.655,.12),(6.18,.63,.49,.11),(6.29,.34,.28,.10),(6.31,.06,.08,.10)]
loft('Head | sculpted cheek and chin silhouette',head_rows,BLUE,'head',exponent=.92)
cylinder('Neck | flexible connector',(0,.02,4.46),.43,.22,BLACK,'neck')
ring('Neck | top collar',(0,.02,4.48),.48,.37,.055,BLACK,'neck')
loft('Pelvis | rounded blue casing',[(2.12,.33,.26,.03),(2.20,.66,.47,.03),
    (2.37,.91,.59,.03),(2.58,1.005,.625,.03),(2.67,.98,.61,.03)],BLUE,'hips',exponent=.90)
...
border=[(-w,front_surface(-w,z)-.003,z) for z,w in panel_rows]
border += [(w,front_surface(w,z)-.003,z) for z,w in reversed(panel_rows)]
pipe('Belly | fine perimeter gasket',border,.014,SEAM,'torso',closed=True)
...
for i,z in enumerate((3.02,3.48,3.88),1):
    cylinder('Button %02d | black fastener'%i,(0,front_surface(0,z)-.031,z),.079,.040,BLACK,'torso',axis=(0,-1,0))
# Fitted lower pale pelvis inset, not a floating rectangular bib.
prism('Pelvis | lower ice-blue inset',[(-.52,2.62),(.52,2.62),(.35,2.34),(.13,2.23),(-.13,2.23),(-.35,2.34)],-.596,.048,PALE,'hips',.032)

# Bow tie with two flared lobes and a rounded central knot.
...
# Sculpted muzzle and actual separate mouth / tooth pieces.
...
# Black sunglasses with designed white reflection stripes; removable named pieces.
for s,label in ((-1,'R'),(1,'L')):
    x=s*.357
    ...
    outline=[(x+u,5.744+v) for u,v in shape]
    prism('Sunglasses | frame '+label,outline,-.781,.114,BLACK,'head',.035)
    inset=[(x+u*.865,5.744+v*.865) for u,v in shape]
    prism('Sunglasses | lens '+label,inset,-.804,.025,GLASS,'head',.023)
...
# Rounded ears, softly inset ear disks, over-ear headphones, center emblems.
for s,label in ((-1,'R'),(1,'L')):
    ...
    cylinder('Headphones | soft cushion '+label,(s*.904,.105,5.695),.337,.19,BLACK,'head',axis=(s,0,0))
    cylinder('Headphones | outer cup '+label,(s*1.035,.105,5.695),.32,.135,METAL,'head',axis=(s,0,0))
    cylinder('Headphones | cup face '+label,(s*1.113,.105,5.695),.277,.030,BLACK,'head',axis=(s,0,0))
    cylinder('Headphones | central badge '+label,(s*1.135,.105,5.695),.114,.022,METAL,'head',axis=(s,0,0))
    ring('Headphones | badge rim '+label,(s*1.150,.105,5.695),.113,.113,.011,BLACK,'head',axis=(s,0,0))
...
# Three swept tufts form a dimensional, asymmetric forelock.
def tuft(name, rows):
...
# Back seam is deliberately fine and lies on the rear casing surface.
...
# Small top hat: flattened brim, tapered crown and white ribbon.
...
# Arms, joint gaps, three upper-arm bands, cuffs, charcoal palms and blue fingers.
bone_specs=[]
for s,label in ((-1,'R'),(1,'L')):
    shoulder=(s*1.015,.01,4.245)
    elbow=(s*1.435,.00,3.375)
    ...
    for k,t in enumerate((.235,.36,.485),1):
        c=a+axis*t
        cylinder('Upper arm | pale stripe %d '%k+label,c,.359,.066,PALE,upper,axis=axis)
        for sign in (-1,1):
            ring('Upper arm | stripe edge %d %d '%(k,sign)+label,c+axis.normalized()*(sign*.037),.354,.354,.009,SEAM,upper,axis=axis)
    ...
    cylinder('Wrist | pale cuff '+label,Vector(wrist)-axis2.normalized()*.012,.255,.113,PALE,lower,axis=axis2)
    ring('Wrist | cuff gasket '+label,Vector(wrist)+axis2.normalized()*.049,.248,.248,.012,SEAM,lower,axis=axis2)
...
# Legs with separate thighs, knee pads, ankle cuffs and three pale toe caps each.
for s,label in ((-1,'R'),(1,'L')):
    hip=(s*.60,.025,2.34)
    knee=(s*.716,.02,1.475)
    ankle=(s*.737,-.02,.63)
...
checkpoint('geometry_ready','Blue animatronic shells, face, accessories and articulated limbs created')

# Rigid hard-surface armature. Every generated mesh has one explicit bone weight.
...
core=[('root',(0,0,.1),(0,0,.65),None),('hips',(0,0,2.20),(0,0,2.78),'root'),
    ('torso',(0,0,2.78),(0,0,4.33),'hips'),('neck',(0,0,4.33),(0,0,4.68),'torso'),
    ('head',(0,0,4.68),(0,0,6.22),'neck'),('jaw',(0,-.37,4.90),(0,-.49,4.45),'head')]
...
for obj,bone in parts:
    if obj.type=='MESH':
        ...
        mod.object=rig
        obj.parent=rig
rig['instructions']='Pose Mode: rotate head/jaw, upper_arm, forearm, hand, thigh, shin and foot bones. Shell parts are rigid-weighted.'

# Lighting and portrait camera are saved with the model; bridge adds review cameras.
...
# Useful opening view in interactive Blender.
...
checkpoint('rig_and_studio_ready','Created %d editable mesh parts and %d rigid-control bones; visual review remains pending'%(len(parts),len(rig.data.bones)))
